import { flowCues, mountOrchestrationFlow } from './orchestrationFlow'

type DeckSlideEvent = CustomEvent<{ id: string; direction: number }>

const cues = flowCues.map(cue => cue.label)

export function mountCoordinationNarrative(): () => void {
  const slide = document.querySelector<HTMLElement>('#indexeur')
  const stage = slide?.querySelector<HTMLElement>('.orchestration-stage')
  ...
  const initial = Number(params.get('orch') ?? 0)
  let cue = Number.isInteger(initial) ? Math.max(0, Math.min(cues.length - 1, initial)) : 0
  let wheelLockedUntil = 0
  let lastWheelAt = -Infinity
  const flow = mountOrchestrationFlow(stage)

  const views = () => [stage, ...document.querySelectorAll<HTMLElement>('.pipeline-camera .orchestration-stage')]
  const setCue = (next: number, animate = true, announce = true) => {
    ...
    if (announce && live) live.textContent = `Orchestration, temps ${cue + 1} sur ${cues.length} : ${cues[cue]}.`
  }

  setCue(cue, false, false)
  const settled = () => slide.classList.contains('is-active') && Math.abs(slide.getBoundingClientRect().top) < 3
  const click = (event: MouseEvent) => {
    const target = event.target instanceof Element ? event.target.closest('[data-orch-goto], [data-orch-prev], [data-orch-next]') : null
    if (!target) return
    if (target.hasAttribute('data-orch-goto')) setCue(Number(target.getAttribute('data-orch-goto')))
    else if (target.hasAttribute('data-orch-prev')) setCue(cue - 1)
    else if (target.hasAttribute('data-orch-next')) setCue(cue + 1)
    if (!layoutQuery.matches) {
      const node = stage.querySelector<SVGGraphicsElement>('.of-node.is-current')
      const viewport = stage.querySelector<HTMLElement>('.orch-flow-viewport')
      if (node && viewport) {
  ...
  const nodeKey = (event: KeyboardEvent) => {
    ...
    const target = event.target.closest('[data-orch-goto]')
    if (!target) return
    event.preventDefault()
    event.stopPropagation()
    setCue(Number(target.getAttribute('data-orch-goto')))
  }
  ...
  const key = (event: KeyboardEvent) => {
    ...
    if (event.target instanceof Element && event.target.closest('button, a, input, textarea, select, [data-orch-goto], [contenteditable="true"]')) return
