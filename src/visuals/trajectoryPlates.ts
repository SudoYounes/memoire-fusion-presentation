...
const poseNames = ['Départ','Élévation','Rotation','Approche']
function poseSelector() {
  return `<g class="tp-pose-selector">${data.boundaries.map((b,i)=>`<g class="tp-pose-choice" data-traj-pose="${i}" tabindex="0" role="button" aria-label="Voir la pose ${b.id} : ${poseNames[i]}" transform="translate(${75+i*224} 566)"><rect width="203" height="27" rx="3"/><text x="14" y="19">${b.id} · ${poseNames[i]}</text></g>`).join('')}</g>`
}
...
function geometryRoute() {
  return frame(2,title('Un passage organisé avant l’exécution','Géométrie · 2 / 2')+image('B','data-traj-scene')+route(true)+divider+trajectoryCommentary(2)+poseSelector())
}

const timeX=(t:number)=>215+t/evidence.durationS*716
function profiles() {
  ...
  const profiles=rows.map(row=>{
    ...
    return `<g class="tp-figure-focus" data-tp-focus="profile-${row.key}"><text class="tp-chart-label" x="32" y="${row.y+23}">${row.title}</text>${math(32,row.y+47,({q:'tp_position',qd:'tp_velocity',qdd:'tp_acceleration'} as const)[row.key],19,'tp-note')}
      ${ticks.map(v=>`<path class="tp-grid" d="M215 ${y(v)} H931"/><text class="tp-tick" x="203" y="${y(v)+4}" text-anchor="end">${v}</text>`).join('')}
  ...
  return frame(3,title('Les points de passage deviennent des lois de mouvement','Temporisation · 1 / 2')+
    ...
    `<text class="tp-note" x="215" y="583">J1 · azimut du robot · mêmes repères A–B–C–D</text>`+divider+
    trajectoryCommentary(3))
}

function limits() {
  const rows=[['Vitesse',evidence.dynamicRatios.maximum_velocity_ratio,'J1 / J4'],['Accélération',evidence.dynamicRatios.maximum_acceleration_ratio,'J1'],['Jerk',evidence.dynamicRatios.maximum_jerk_ratio,'J2']] as const
  return frame(4,title('Les limites dynamiques contraignent la durée','Temporisation · 2 / 2')+
    `<text class="tp-note" x="32" y="268">Maximum atteint / limite déclarée · tous axes · ce transfert</text>`+
    rows.map(([label,ratio,joint],i)=>{const y=324+i*93;return `<g class="tp-figure-focus" data-tp-focus="limit-${['velocity','acceleration','jerk'][i]}"><text class="tp-heading" x="32" y="${y}">${label}</text><text class="tp-note" x="32" y="${y+26}">Axe dimensionnant : ${joint}</text><rect class="tp-bar-track" x="265" y="${y-16}" width="570" height="19" rx="3"/><rect class="tp-bar ${i?'tp-bar--near':''}" x="265" y="${y-16}" width="${570*ratio}" height="19" rx="3"/>${math(954,y+2,`tp_ratio_${['velocity','acceleration','jerk'][i]}` as RuntimeMathKey,28,'tp-ratio',false,'end')}</g>`}).join('')+
    `<path class="tp-limit-line" d="M835 287 V530"/><text class="tp-note" x="835" y="559" text-anchor="middle">100 % : limite du modèle</text>`+divider+
    trajectoryCommentary(4))
}
...
function validation() {
  ...
  return frame(5,title('Vérifier le passage dans la scène actualisée','MoveIt 2 · /check_state_validity')+`<g class="tp-figure-focus tp-scene-focus" data-tp-focus="validation-scene">${image('C')}</g>`+route()+`<g class="tp-figure-focus" data-tp-focus="validation-samples">${dots}</g>`+divider+trajectoryCommentary(5)+
    text(32,583,['Points affichés espacés pour la lecture · le contrôle porte sur les 256 états'],'tp-note'))
}

function output() {
  return frame(6,title('Une consigne articulée et horodatée pour le contrôleur','Sortie · RobotTrajectory')+
...
export const trajectoryPlatesMarkup = () => `<defs><clipPath id="tp-area"><rect x="-8" y="176" width="1616" height="432"/></clipPath></defs>
  ...
  <g clip-path="url(#tp-area)" class="tp-panels">${overview()}${geometryTarget()}${geometryRoute()}${profiles()}${limits()}${validation()}${output()}</g>`
...
export function updateTrajectoryPlates(view: HTMLElement, cue: number) {
  ...
  view.querySelectorAll<SVGElement>('[data-tp-origin]').forEach(el=>el.classList.toggle('is-current',cue>0&&Number(el.dataset.tpOrigin)===(cue<3?0:cue<5?1:2)))
}
